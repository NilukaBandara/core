...
ALEXA_SECURITY_CONTROLLER = "Alexa.SecurityPanelController"
HANDLERS: Registry[
    tuple[str, str],
    Callable[
...
@HANDLERS.register((ALEXA_SECURITY_CONTROLLER, "Arm"))
async def async_api_arm(
    ...
    response = directive.response(
        name="Arm.Response", namespace=ALEXA_SECURITY_CONTROLLER, payload=payload
    )

    response.add_context_property(
        {
            "name": "armState",
            "namespace": ALEXA_SECURITY_CONTROLLER,
            "value": arm_state,
        }
    )
...
@HANDLERS.register((ALEXA_SECURITY_CONTROLLER, "Disarm"))
